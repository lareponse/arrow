...
function map_introspect(string $table): array
{
    ...
    foreach ($columns as $col) {
        ...
        if (str_contains($col['Extra'], 'GENERATED')
...
function map_read(string $table, mixed $id, bool $include_disabled = false): array
{
    $schema = map_schema($table);
...
function map_delete(string $table, mixed $id, bool $soft = true): int
{
    ...
    if ($soft) {
        return map_update($table, $conditions, ['disabled_at' => date('Y-m-d H:i:s')]);
    }
...
{
    $options += ['limit' => null, 'offset' => 0, 'order' => 'created_at DESC', 'include_disabled' => false];
...
{
    return map_update($table, $id, ['published_at' => $when ?: date('Y-m-d H:i:s')]);
}

function map_unpublish(string $table, mixed $id): int
{
    return map_update($table, $id, ['published_at' => null]);
}
