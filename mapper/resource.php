...
function resource_create($data)
{
    ...
    $stmt = dbq(db(), ...qb_create('resources', null, $insert_data));
...
function resource_update($id, $data)
{
    ...
    $stmt = dbq(db(), ...qb_update('resources', $updateData, 'id = ?', [$id]));
...
function resource_delete($id)
{
    ...
    $resource = dbq(db(), "SELECT file_path FROM resources WHERE id = ?", [$id])->fetch();

    if (!$resource) {
        return false;
    }

    $stmt = dbq(db(), "DELETE FROM resources WHERE id = ?", [$id]);
