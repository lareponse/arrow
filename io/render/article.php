...
<section class="articles-section" aria-labelledby="articles-title">
    ...
    <div class="masonry" id="articlesContainer">
        ...
        <?php foreach ($articles_events as $i => $item): ?>
            ...
            // vd($item); 
            ?>
            <article class="card <?= ($i === 0) ? 'card-wide featured' : '' ?>" data-type="<?= $item['type'] ?>" data-date="2025-01-15">
                ...
                <img src="<?= $item['avatar'] ?? '/static/assets/hero.jpeg' ?>" alt="<?= $item['title'] ?? '' ?>" loading="lazy">
                <div class="card-content">
                    <div class="article-meta">
                        <span class="article-category"><?= $item['category_label'] ?? '' ?></span>
                    ...
                    <?php if ($item['type'] === 'event' || $item['type'] === 'webinar'): ?>
                        <div class="event-details">
                            ...
                            <?php if (isset($item['duration_minutes'])): ?>
                                <p><strong>Durée :</strong> <?= $item['duration_minutes'] ?> minutes</p>
                            <?php endif; ?>
                            <?php if (isset($item['places_max'])): ?>
                                <p><strong>Places :</strong> <?= $item['places_max'] ?></p>
                            <?php endif; ?>
                            <?php if (isset($item['price_ht'])): ?>
                                <p><strong>Prix :</strong> <?= $item['price_ht'] ?> €</p>
                            <?php endif; ?>
                        </div>
                        <a href="/article/detail/<?= $item['slug']; ?>" class="cta">Voir l'evenement complet</a>
                    <?php else: ?>
                        ...
                        <a href="/article/detail/<?= $item['slug']; ?>" class="cta">Lire l'article complet</a>
